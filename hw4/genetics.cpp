...
vector<vector<int>> buildCommonFragments(
	...
	for (int start = 0; start < n; ++start) {
		...
		// Common edges can form cycles; cut the weakest edge in cycle to produce one path fragment.
...
int main() {
	...
	vector<int> bestOrder = solveWithGenetic(points);

	long double bestCost = tourLengthIdx(bestOrder, points);

	cout << fixed << setprecision(10) << (double)bestCost << '\n';
	for (int i = 0; i < n; ++i) {
		if (i) {
			cout << ' ';
		}
		cout << points[bestOrder[i]].id;
	}
	cout << '\n';
	return 0;
}
